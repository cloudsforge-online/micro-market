...
# and for this service the reason is two constraints rather than one:
#
#   * `listings_active_is_escrowed` — an active listing must name the ledger reservation that holds
#     its item. Below SCHEMA_VERSION it may not exist, and a listing could be opened for business
#     holding nothing at all.
#   * `orders_listing_uniq` — one listing yields at most one order, for ever. Without it the same
#     item can be sold to two buyers, and only one of them can be given it.
#
# A service that could create those at boot is a service that could start without them.
