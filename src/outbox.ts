 ...
 *
 * `market.listing.sold` is read by worlds, billing, ledger, activity, notify and analytics
 * (07-dependency-map §events). It is the event that hands a game item to its new owner and that
 * grants a platform-native entitlement — AD-14's third row, "ledger + entitlement grant, no
 * chain". If it is published after the commit and the process dies in between, a buyer has paid
 * and received nothing, and nothing in the estate knows. If it is published before the commit and
 * the transaction rolls back, a buyer receives an item they did not buy. Writing the event in the
 * same transaction as the order is the only arrangement in which neither is possible.
 */
 ...
 *   - **`actor` falls back to `service:market`.** An emit with no actor was this service acting on
 *     its own behalf — a job, a sweep — and that is precisely what `serviceActor` spells. `null`
 *     is not an actor the contract has a word for.
 */
export function buildEnvelope(row: OutboxRow): EventEnvelope {
  return {
    ...
    actor: row.actor ?? serviceActor('market'),
