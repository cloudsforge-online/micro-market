 ...
 * instead of a hand-maintained list. Nothing had ever compared those against the registry, and
 * `POLICY_SCOPES` had said `policy:evaluate` — a scope that has never existed — for the life of
 * the service.
 *
 ...
 * test pinning `POLICY_SCOPES` to `['policy:decide']` would pass just as happily if a new client
 * were added tomorrow naming `custody:sign` or `market:evaluate`; this one goes red. So the
 ...
 */
const AT_LEAST_THIS_MANY_CONSTANTS = 3
...
test('the sweep finds the outbound constants it is supposed to be checking', async () => {
  ...
  for (const expected of ['POLICY_SCOPES', 'LEDGER_SCOPES', 'INDEXER_SCOPES']) {
...
test('the sweep can fail: it reads declarations, not prose, and values, not text', () => {
  ...
  assert.equal(isScope('policy:decide'), true)
  assert.equal(isScope('policy:evaluate'), false)
})
