 ...
 *
 * market serves BOTH estates from one process since the network consolidation (micro-deploy
...
describe('the handle a request gets', () => {
  ...
  it('REFUSES when this deployment holds no handle for that network', () => {
    ...
    // write a testnet buyer's purchase into the mainnet database AND post it to the mainnet
    // ledger, leaving matching rows on both sides to say it was intended.
...
describe('the operational endpoints are exempt, and only they', () => {
  ...
  it('does not exempt anything that reads or writes', () => {
    for (const p of ['/v1/listings', '/v1/orders', '/v1/bids']) {
